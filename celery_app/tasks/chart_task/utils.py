...
def get_week_dates(from_date_str, to_date_str, zone_name):
    ...
    start_date = datetime.strptime(
        from_date_str + " 00:00:00", "%Y-%m-%d %H:%M:%S")
    end_date = datetime.strptime(
        to_date_str + " 23:59:59", "%Y-%m-%d %H:%M:%S")
...
def convert_str_time(seconds):
    ...
    if hour > 0:
        if hour > 1:
            res_str += '{}hrs, '.format(hour)
        else:
            res_str += '{}hr, '.format(hour)
    res_str += '{}m, {}s'.format(minutes, seconds)
    return res_str
...
def prepare_report_data(visitors_summmary_info, header_info, chart_info, footer_info, base_url):
    ...
    report_data['events'] = []
    visitors_trend_title = ["Current Week", "Previous Week",
                            "Same Week, Previous Month", "Same Week, Previous Year"]
    ...
    for i, visitor_data in enumerate(visitors_summmary_info):
        if len(visitor_data['analytic_data']) > 0:
            if i == 0:
                ...
                report_data['visitors_count'] = [format(
                    i, ',d') for i in visitor_data['analytic_data'][0]['series'][3]['total_visitor']]
            ...
            visitor_info['count'] = format(
                visitor_data['analytic_data'][0]['series'][2]['value'], ',d')
            visitor_info['avg_change'] = str(
                visitor_data['analytic_data'][0]['series'][2]['avg_change']) + '%'
